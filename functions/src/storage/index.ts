 ...
 */
async function storeImageToFirebase(
    imageUrl: string,
    path: string,
 ...
 */
export const storeImage = functions
    .runWith({
        timeoutSeconds: 60,
        memory: '512MB'
    })
 ...
 */
export const storeImages = functions
    .runWith({
    ...
    try {
        ...
        for (let i = 0; i < images.length; i += concurrencyLimit) {
            ...
            const batchPromises = batch.map(async (img: { url: string; type: string }, index: number) => {
                try {
                    const timestamp = Date.now();
            ...
            results.push(...batchResults.filter((r): r is StoredImage => r !== null));
        }
 ...
 */
export const storeBase64Image = functions
    .runWith({
        timeoutSeconds: 60,
        memory: '512MB'
    })
 ...
 */
export const deleteSiteImages = functions
    .runWith({
        timeoutSeconds: 60,
        memory: '256MB'
    })
