...
import { checkRateLimit, getClientIdentifier, rateLimitResponse } from '../utils/rateLimiter';
import { createTracker } from '../utils/logger';
...
async function getCachedIdentity(url: string): Promise<SiteIdentity | null> {
    try {
        ...
        return data.identity as SiteIdentity;
    } catch (error) {
...
async function setCachedIdentity(url: string, identity: SiteIdentity): Promise<void> {
    try {
        const hash = getUrlHash(url);
 ...
 */
function shouldSkipNavLink(href: string, label: string, baseHostname: string): boolean {
 ...
 */
function assessContentSparsity(fullCopy: string): 'rich' | 'moderate' | 'sparse' {
    const charCount = fullCopy.length;
    if (charCount < 500) return 'sparse';
    if (charCount < 1500) return 'moderate';
    return 'rich';
}
 ...
 */
function extractLogo($: cheerio.CheerioAPI, sourceUrl: string): string | null {
 ...
 */
function isProductServiceImage(src: string, alt: string, context: string): boolean {
    const srcLower = src.toLowerCase();
    ...
    const hasProductIndicator = productIndicators.some(p =>
        srcLower.includes(p) || altLower.includes(p) || contextLower.includes(p)
    );
 ...
 */
function extractProductImages($: cheerio.CheerioAPI, sourceUrl: string, logoUrl: string | null): ProductImage[] {
    ...
    for (const { selector, context } of prioritySelectors) {
        ...
        for (const el of elements) {
            ...
            if (!src || shouldSkipImage(src)) continue;
            ...
            if (!isProductServiceImage(src, alt, fullContext)) continue;
...
function extractNavigation($: cheerio.CheerioAPI, sourceUrl: string): NavigationLink[] {
    ...
    for (const selector of navSelectors) {
        const elements = $(selector).toArray();
        for (const el of elements) {
            if (links.length >= 6) break; // Cap at 6 items

            const href = $(el).attr('href');
            const label = $(el).text().trim();

            if (!href || !label || label.length < 2 || label.length > 30) continue;
            if (seenLabels.has(label.toLowerCase())) continue;
            if (shouldSkipNavLink(href, label, baseHostname)) continue;
 ...
 */
function extractSocialLinks($: cheerio.CheerioAPI): SiteIdentity['socialLinks'] {
    const social: SiteIdentity['socialLinks'] = {};
    ...
    for (const selector of searchAreas) {
        ...
        if (Object.keys(social).length > 0) break;
    }
 ...
 */
function extractContactInfo($: cheerio.CheerioAPI): SiteIdentity['contactInfo'] {
    const contact: SiteIdentity['contactInfo'] = {};
 ...
 */
function extractFullCopy($: cheerio.CheerioAPI): string {
    ...
    $content('main, article, section, .content, #content, [role="main"], .main, #main').each((_, el) => {
        ...
        $content(el).find('h1, h2, h3').each((_, h) => {
            const tag = (h as { tagName?: string }).tagName?.toLowerCase() || 'h2';
...
function extractBusinessName($: cheerio.CheerioAPI, sourceUrl: string): string {
    ...
    const isNoisyName = (name: string): boolean => {
 ...
 */
async function analyzeVisualVibe(page: any): Promise<{ vibe: string; screenshot: string }> {
    try {
 ...
 */
async function extractColorsAdvanced(
    page: any,
    logoUrl: string | null,
    sourceUrl: string
): Promise<string[]> {
    ...
    if (allColors.length < 2 && logoUrl) {
        try {
            ...
            if (palette.Vibrant) logoColors.push(palette.Vibrant.hex.toLowerCase());
            if (palette.DarkVibrant) logoColors.push(palette.DarkVibrant.hex.toLowerCase());
            if (palette.Muted && logoColors.length < 3) logoColors.push(palette.Muted.hex.toLowerCase());
        ...
        try {
            const pageColors = await page.evaluate(() => {
                ...
                elements.forEach(el => {
                    ...
                    if (bgHex) foundColors.push(bgHex.toLowerCase());
                    if (textHex) foundColors.push(textHex.toLowerCase());
                    if (borderHex) foundColors.push(borderHex.toLowerCase());
                });
 ...
 */
function applyFallbacks(identity: SiteIdentity): SiteIdentity {
 ...
 */
export async function extractSiteIdentityCore(
    url: string,
 ...
 */
export const extractSiteIdentity = onRequest(
    {
    ...
    async (req, res) => {
        const tracker = createTracker('extractSiteIdentity');
        ...
        try {
            ...
            const rateLimit = await checkRateLimit(clientId, 'extractSiteIdentity');
            if (!rateLimit.allowed) {
                rateLimitResponse(res, rateLimit);
                return;
            }
