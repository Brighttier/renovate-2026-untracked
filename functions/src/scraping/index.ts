 ...
 */
async function getCachedScrape(url: string): Promise<ExtractedAssets | null> {
    try {
        ...
        const cacheTTLDays = config?.cacheTTLDays || DEFAULT_CACHE_TTL_DAYS;
        ...
        return data.assets as ExtractedAssets;
    } catch (error) {
 ...
 */
async function setCachedScrape(url: string, assets: ExtractedAssets): Promise<void> {
    try {
        ...
        await db.collection(CACHE_COLLECTION).doc(hash).set({
            ...
            expiresAt: new Date(Date.now() + (config?.cacheTTLDays || DEFAULT_CACHE_TTL_DAYS) * 24 * 60 * 60 * 1000)
        });
    } catch (error) {
 ...
 */
function classifyImage(alt: string, src: string, context?: string): ExtractedImage['type'] {
 ...
 */
async function extractColorsFromPage(page: puppeteer.Page): Promise<string[]> {
    return page.evaluate(() => {
        ...
        elements.forEach(el => {
            ...
            if (bgHex) colors.add(bgHex);
            if (textHex) colors.add(textHex);
        });
 ...
 */
export const scrapeWebsite = onRequest(
    {
    ...
    async (req, res) => {
        const tracker = createTracker('scrapeWebsite');
        ...
        try {
            ...
            const rateLimit = await checkRateLimit(clientId, 'scrapeWebsite');
            ...
            try {
                ...
                $('img').each((_, el) => {
                    ...
                    if (imageType === 'logo' && result.logo === absoluteUrl) return;
