 ...
 */
export const findBusinesses = functions.https.onRequest(async (req, res) => {
 ...
 */
export const generateBlueprint = functions.https.onRequest(async (req, res) => {
    // Handle CORS preflight
    if (req.method === 'OPTIONS') {
        res.set(corsHeaders).status(204).send('');
        return;
...
${systemInstruction}`;
        ...
        if (jsonMatch) {
            ...
            if (blueprint.sections && Array.isArray(blueprint.sections)) {
                blueprint.sections = blueprint.sections.map((section: any, index: number) => ({
                    ...section,
...
// Intent types for targeted prompts
type EditIntent = 'CHANGE_COLOR' | 'CHANGE_STYLE' | 'ADD_SECTION' | 'REMOVE_SECTION' | 'UPDATE_TEXT' | 'CHANGE_LAYOUT' | 'CHANGE_FONT' | 'UPDATE_NAVBAR' | 'UPDATE_FOOTER' | 'REORDER_SECTIONS' | 'GENERAL_EDIT';

// Analyze intent from instruction
function analyzeIntent(instruction: string): { intent: EditIntent; confidence: number } {
...
function getIntentInstructions(intent: EditIntent): string {
    switch (intent) {
        case 'CHANGE_COLOR':
 ...
 */
export const editBlueprint = functions.https.onRequest(async (req, res) => {
    // Handle CORS preflight
    if (req.method === 'OPTIONS') {
        res.set(corsHeaders).status(204).send('');
        return;
 ...
 */
export const generateSiteHTML = functions.https.onRequest(async (req, res) => {
    // Handle CORS preflight
    if (req.method === 'OPTIONS') {
        res.set(corsHeaders).status(204).send('');
        return;
...
- Trust badge: Show rating (${researchData?.rating || '4.9'}/5) with star icons
- Social proof: "${researchData?.reviewCount || '500'}+ happy customers"
- Background: Industry-appropriate image with dark overlay
${researchData?.photos && researchData.photos[0] ? `- Use this image: ${researchData.photos[0]}` : ''}

## 3. SERVICES/FEATURES SECTION
Create visually striking cards for each service:
${researchData?.services ? researchData.services.slice(0, 6).map((s: string, i: number) => `${i + 1}. ${s}`).join('\n') : `1. Primary Service\n2. Secondary Service\n3. Additional Service`}
...
- ${researchData?.yearEstablished ? `Highlight: "Serving since ${researchData.yearEstablished}"` : 'Years of experience'}
...
- Business contact info: ${researchData?.phone || 'Phone'}, ${address || 'Address'}
- ${researchData?.hours ? `Hours: ${researchData.hours}` : 'Business hours display'}
        ...
        if (isLogoReplacement && attachments && attachments.length > 0) {
            ...
            if (!replaced) {
                ...
                if (iconPattern.test(updatedHtml)) {
                    updatedHtml = updatedHtml.replace(
                        iconPattern,
                        `$1<img src="${dataUri}" alt="Logo" class="h-12 w-auto object-contain max-w-[180px]">`
                    );
                    replaced = true;
            ...
            if (!replaced) {
                ...
                if (textLogoPattern.test(updatedHtml)) {
                    updatedHtml = updatedHtml.replace(
                        textLogoPattern,
                        `$1<img src="${dataUri}" alt="Logo" class="h-12 w-auto object-contain max-w-[180px]">$3`
                    );
                    replaced = true;
...
// ============================================
// SITE MODERNIZATION (New Pipeline)
// ============================================

// ProductImage type for image enhancement
...
// SiteIdentity type definition (matches types.ts)
interface SiteIdentity {
    ...
    productImages: ProductImage[];  // Top 3 product/service images for enhancement
    socialLinks: {
        facebook?: string;
        instagram?: string;
        twitter?: string;
 ...
 */
async function _callExtractSiteIdentity(url: string, forceRefresh: boolean): Promise<SiteIdentity> {
 ...
 */
async function generateImageEditPrompts(
    ...
    try {
        ...
        if (jsonMatch) {
            ...
            return productImages.map((img, i) => {
                const promptData = prompts.find((p: { index: number }) => p.index === i);
                return {
                    ...img,
                    editPrompt: promptData?.editPrompt ||
 ...
 */
async function enhanceImageWithNanoBanana(
    originalUrl: string,
    editPrompt: string,
    colorPalette?: string[]
): Promise<string | null> {
 ...
 */
async function _enhanceProductImages(
    productImages: ProductImage[],
    businessName: string,
    category: string,
    visualVibe: string,
    ...
    const imagesWithPrompts = await generateImageEditPrompts(
        productImages,
        businessName,
        category,
        visualVibe
    ...
    const enhancedImages = await Promise.all(
        imagesWithPrompts.map(async (img) => {
            if (!img.editPrompt) {
                return img;
            }

            const enhancedUrl = await enhanceImageWithNanoBanana(
                img.originalUrl,
                img.editPrompt,
                colorPalette
            );
...
/**
 * BrandManifest - Lightweight structured output from the Lite Scout
 * Contains only essential brand identity data for subsequent steps
 */
...
/**
 * SiteBlueprint - Section plan from the Thinking Architect
 * Describes what sections to generate without actual HTML
 */
interface SiteBlueprint {
    designStyle: 'saas-modern' | 'bento-grid' | 'high-end-minimal';
    sections: {
        id: string;
        type: 'hero' | 'services' | 'about' | 'testimonials' | 'contact' | 'cta' | 'features' | 'gallery' | 'faq';
        priority: number;
        contentHints: string;
        imageNeeded: boolean;
    }[];
...
/**
 * GeneratedSection - Output from parallel component generation
 */
 ...
 */
async function extractBrandManifest(
...
Return ONLY a JSON object with this exact structure (no markdown, no explanation):
{
  ...
  "contactAddress": "address if found"
    ...
    try {
        ...
        const manifest = JSON.parse(jsonMatch[0]) as BrandManifest;
 ...
 */
async function createSiteBlueprint(
...
Optional sections based on content: about, testimonials, features, gallery, faq, cta`;
    ...
    try {
        ...
        const blueprint = JSON.parse(jsonMatch[0]) as SiteBlueprint;
...
async function generateSection(
    sectionConfig: SiteBlueprint['sections'][0],
    manifest: BrandManifest,
    blueprint: SiteBlueprint,
    enhancedImages: ProductImage[]
): Promise<GeneratedSection> {
    console.log(`[Pipeline:Generator] Generating section: ${sectionConfig.id}`);
    const apiKey = await getGeminiApiKey();

    // Find relevant image for this section
    const sectionImage = enhancedImages.find(img =>
        img.context === sectionConfig.type ||
 ...
 */
async function generateSectionsParallel(
    blueprint: SiteBlueprint,
    manifest: BrandManifest,
    enhancedImages: ProductImage[],
    ...
    while (queue.length > 0) {
        ...
        const batchPromises = batch.map(section =>
            generateSection(section, manifest, blueprint, enhancedImages)
        );
 ...
 */
async function _runModularPipeline(
    siteIdentity: SiteIdentity,
    category: string,
    enhancedImages: ProductImage[]
): Promise<{ html: string; manifest: BrandManifest; blueprint: SiteBlueprint; thinking: string }> {
...
// Types for modernization
type ModernizationStyle = 'saas-modern' | 'bento-grid' | 'high-end-minimal' | 'auto';

interface ModernizationRequest {
    sourceUrl: string;
    businessName?: string;
    category?: string;
 ...
 */
function _determineStyleFromVibeAndCategory(vibe: string, category: string): ModernizationStyle {
 ...
 */
function _extractThought(text: string): string {
 ...
 */
function _extractCodeUpdate(text: string): string {
...
import { runPostProcessingPipeline } from '../processing/postProcessor';
...
async function generatePremiumModernizedSite(
    ...
    const heroImageCount = semanticMap?.hero?.length || siteIdentity.heroImages?.length || 0;
    const serviceImageCount = semanticMap?.services?.length || 0;
...
`;
    ...
    const extractedFacts = (siteIdentity as any).extractedFacts || [];
    ...
    const semanticCaptions = [
        ...(semanticMap?.hero?.map((img: any, i: number) => `[[ID_HERO_${i + 1}_HERE]]: ${img.semanticCaption || img.alt || 'Hero image'}`) || []),
        ...(semanticMap?.services?.map((img: any, i: number) => `[[ID_SERVICE_IMG_${i + 1}_HERE]]: ${img.semanticCaption || img.alt || 'Service image'}`) || []),
        ...(semanticMap?.gallery?.map((img: any, i: number) => `[[ID_GALLERY_${i + 1}_HERE]]: ${img.semanticCaption || img.alt || 'Gallery image'}`) || []),
    ].join('\n');
...
- Accent Color: ${(siteIdentity as any).accentColor || 'use primaryColors[2]'}

## Brand Colors (USE ALL via CSS variables)
Primary: ${siteIdentity.primaryColors?.[0] || '#3B82F6'}
...
${siteIdentity.fullCopy?.slice(0, 3000) || 'No additional content'}
...
8. ${hasGeneratedImages ? 'CRITICAL: The hero section MUST use the AI-generated hero image URL. Service sections MUST use service image URLs.' : 'Use placeholder images that will be injected later'}`;
    ...
    if (codeMatch) {
        ...
        console.log('[PremiumGen v3.0] Found code blocks:', allCodeBlocks?.length || 0);
    ...
    const { html: processedHtml, validation } = runPostProcessingPipeline(html, siteIdentity as any);
...
export const generateModernizedSite = functions
    ...
    try {
        ...
        const siteIdentity = await deepScrapeSite(sourceUrl, {
        ...
        console.log('[ModernizeSite v3.0] Deep-multimodal scrape complete:', {
            businessName: siteIdentity.businessName,
            pages: siteIdentity.pages?.length || 0,
            services: siteIdentity.services?.length || 0,
            testimonials: siteIdentity.testimonials?.length || 0,
            teamMembers: siteIdentity.teamMembers?.length || 0,
            colors: siteIdentity.primaryColors,
            accentColor: (siteIdentity as any).accentColor,
            hasLogo: !!siteIdentity.logoUrl || !!siteIdentity.logoBase64,
            heroImages: siteIdentity.heroImages?.length || 0,
        ...
        const identityForResponse = {
            ...
            pages: siteIdentity.pages?.map(p => ({
                url: p.url,
                title: p.title,
                path: p.path
            })),
            // Include v3.0 metadata
            extractedFacts: (siteIdentity as any).extractedFacts,
            accentColor: (siteIdentity as any).accentColor,
            visionAnalysisComplete: (siteIdentity as any).visionAnalysisComplete
        };
...
import { exhaustiveScrapeSite } from '../scraping/exhaustiveScraper';
import type { UniversalBusinessDNA, TotalContentConfig } from '../../../types';
 ...
 */
function buildTotalContentDataSection(dna: UniversalBusinessDNA): string {
...
`;
    ...
    const semanticCaptions = [
        ...dna.semanticImageMap.hero.map((img, i) =>
            `[[ID_HERO_${i + 1}_HERE]]: ${img.semanticCaption || img.alt || 'Hero image'}`),
        ...dna.semanticImageMap.services.map((img, i) =>
            `[[ID_SERVICE_IMG_${i + 1}_HERE]]: ${img.semanticCaption || img.alt || 'Service image'}`),
        ...dna.semanticImageMap.gallery.slice(0, 10).map((img, i) =>
            `[[ID_GALLERY_${i + 1}_HERE]]: ${img.semanticCaption || img.alt || 'Gallery image'}`),
    ].join('\n');
...
## Business Hours
${dna.consolidatedFooter.businessHours?.formatted || 'Not specified'}
 ...
 */
async function generateTotalContentSite(
    dna: UniversalBusinessDNA
): Promise<{ html: string; thinking: string }> {
    const apiKey = await getGeminiApiKey();
    ...
    const siteIdentity = {
        ...
        businessHours: dna.consolidatedFooter.businessHours?.formatted,
        visualVibe: dna.visualVibe,
        contentSparsity: dna.contentSparsity,
        extractedFacts: dna.hiddenGems.map(g => ({
            source: g.type === 'founding_date' ? 'logo' as const :
                    g.type === 'award' ? 'flyer' as const :
                    g.type === 'slogan' ? 'hero' as const : 'signage' as const,
            text: g.text,
 ...
 */
export const generateTotalContentModernizedSite = functions
    .runWith({
    ...
    try {
        ...
        const dna = await exhaustiveScrapeSite(sourceUrl, config);
 ...
 */
export const analyzeBrand = functions.runWith({
    timeoutSeconds: 540,
    memory: '1GB'
}).https.onRequest(async (req, res) => {
    ...
    try {
        ...
        console.log('[VibeCoder] Analyzing brand for:', businessInfo?.substring(0, 100));
...
`;
        ...
        if (!response.ok) {
            ...
            throw new Error(data.error?.message || 'Brand analysis API failed');
        }
 ...
 */
export const generateSiteFromBrief = functions.runWith({
    timeoutSeconds: 540,
    memory: '1GB'
}).https.onRequest(async (req, res) => {
    // Set CORS headers FIRST for ALL responses (success, error, validation failures)
    ...
    try {
        ...
        console.log('[VibeCoder] Generating site from Design Brief, length:', designBrief?.length);
...
`;
        ...
        if (!response.ok) {
            ...
            throw new Error(data.error?.message || 'Site generation API failed');
        }
        ...
        console.log('[VibeCoder] Extracted code length:', code?.length || 0);
 ...
 */
export const generateVibeImage = functions.runWith({
    timeoutSeconds: 540,
    memory: '1GB'
}).https.onRequest(async (req, res) => {
    // Set CORS headers FIRST for ALL responses (success, error, validation failures)
    ...
    try {
        ...
        console.log('[VibeCoder] Generating image for prompt:', enhancedPrompt?.substring(0, 100));
