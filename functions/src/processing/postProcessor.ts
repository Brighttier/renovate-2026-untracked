...
import type { SiteIdentity, PlaceholderRegistry, ExtractedImage } from '../../../types';
 ...
 */
export function createPlaceholderRegistry(siteIdentity: SiteIdentity): PlaceholderRegistry {
  ...
  // Map hero images
  const heroImages = siteIdentity.semanticImageMap?.hero || siteIdentity.heroImages || [];
  ...
  // Map service images
  const serviceImages = siteIdentity.semanticImageMap?.services || [];
  ...
  // Map gallery images
  const galleryImages = siteIdentity.semanticImageMap?.gallery || siteIdentity.galleryImages || [];
  ...
  // Map team images
  const teamImages = siteIdentity.semanticImageMap?.about || [];
  const teamMemberImages = siteIdentity.teamMembers
    ?.filter(m => m.imageUrl)
    .map(m => ({ url: m.imageUrl! })) || [];
  ...
  allTeamImages.forEach((img, index) => {
    registry.teamImages.push({
      ...
      target: (img as ExtractedImage).base64 || (img as ExtractedImage).url || (img as { url: string }).url || null,
    });
  });
 ...
 */
export function injectHeroImages(
  html: string,
  heroImages: Array<{ url?: string; base64?: string }>
): string {
 ...
 */
export function injectServiceImages(
  html: string,
  serviceImages: Array<{ url?: string; base64?: string }>,
  fallbackImages: Array<{ url?: string; base64?: string }> = []
): string {
 ...
 */
export function injectGalleryImages(
  html: string,
  galleryImages: Array<{ url?: string; base64?: string }>
): string {
 ...
 */
export function injectTeamImages(
  html: string,
  teamImages: Array<{ url?: string; base64?: string; imageUrl?: string }>
): string {
  return html.replace(PLACEHOLDER_PATTERNS.teamImage, (match, indexStr) => {
    ...
    return (img as ExtractedImage).base64 || (img as ExtractedImage).url || (img as { imageUrl?: string }).imageUrl || '';
  });
}
 ...
 */
export function injectRealAssets(
  html: string,
  ...
  const teamImages = siteIdentity.semanticImageMap?.about || [];
  const teamMemberImages = siteIdentity.teamMembers?.filter(m => m.imageUrl) || [];
 ...
 */
export function validateNoPlaceholders(html: string): {
  valid: boolean;
  remaining: string[];
  count: number;
} {
 ...
 */
export function injectAnimationStyles(html: string): string {
 ...
 */
export function injectGlassGlowStyles(html: string): string {
 ...
 */
export function injectSemanticImages(
  html: string,
  semanticImageMap: {
    hero?: Array<{ url?: string; base64?: string; semanticCaption?: string }>;
    services?: Array<{ url?: string; base64?: string; semanticCaption?: string }>;
    about?: Array<{ url?: string; base64?: string; semanticCaption?: string }>;
    gallery?: Array<{ url?: string; base64?: string; semanticCaption?: string }>;
  }
): string {
  let result = html;
