...
const _nodeFetchType: NodeFetchRequestInit | undefined = undefined;
void _nodeFetchType;
...
export async function generateLogo(params: {
  ...
  try {
    ...
    const promptDesigner = genAI.getGenerativeModel({ model: 'gemini-3-pro-preview' });
 ...
 */
export async function uploadGeneratedLogo(
  imageDataUrl: string,
  businessName: string,
  option: number
): Promise<string> {
