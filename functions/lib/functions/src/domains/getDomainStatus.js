...
exports.getDomainStatus = void 0;
...
async function dnsLookup(domain, recordType) {
    try {
        ...
        const response = await fetch(`https://dns.google.com/resolve?name=${encodeURIComponent(domain)}&type=${typeCode}`, {
 ...
 */
async function checkTxtVerification(domain, expectedToken) {
    const txtRecords = await dnsLookup(domain, 'TXT');
    console.log(`TXT records for ${domain}:`, txtRecords);
    return txtRecords.some(record => record.includes(expectedToken) || record.includes('firebase-site-verification'));
}
/**
...
async function checkARecords(domain) {
    ...
    const hasFirebaseIp = aRecords.some(ip => types_1.FIREBASE_HOSTING_IPS.includes(ip));
    return {
 ...
 */
function determineStatus(connection, txtVerified, dnsPropagated) {
...
// Main Cloud Function
// ============================================================================
exports.getDomainStatus = functions
    .runWith({
    timeoutSeconds: 30,
    memory: '256MB'
})
    ...
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'You must be logged in to check domain status');
    }
    ...
    if (connection.userId !== userId) {
        ...
        if (!adminDoc.exists) {
            throw new functions.https.HttpsError('permission-denied', 'You do not have permission to view this domain');
        }
    }
    ...
    if (connection.status === 'connected' || connection.status === 'disconnected') {
        return {
            ...
            sslCertExpiry: connection.sslCertExpiry,
    ...
    const txtVerified = await checkTxtVerification(connection.domain, connection.verificationToken);
    const aRecordCheck = await checkARecords(connection.domain);
    // 6. Determine new status
    const newStatus = determineStatus(connection, txtVerified, aRecordCheck.propagated);
    const now = new Date().toISOString();
    // 7. Determine SSL status
    let sslStatus = connection.sslStatus;
...
//# sourceMappingURL=getDomainStatus.js.map