 ...
 */
async function removeGoDaddyDNSRecords(domain, credentials) {
    try {
        // Get current records
        const getResponse = await fetch(`${GODADDY_API_BASE}/domains/${domain}/records`, {
            headers: {
...
// Main Cloud Function
// ============================================================================
exports.removeDomain = functions
    .runWith({
    timeoutSeconds: 60,
    memory: '256MB'
})
    ...
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'You must be logged in to remove a domain');
    }
    ...
    if (connection.userId !== userId) {
        ...
        if (!adminDoc.exists) {
            throw new functions.https.HttpsError('permission-denied', 'You do not have permission to remove this domain');
        }
        isAdmin = true;
    }
...
//# sourceMappingURL=removeDomain.js.map