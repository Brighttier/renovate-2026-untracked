...
exports.findLeadsWithMaps = exports.findBusinessesWithSearch = exports.researchBusiness = void 0;
const functions = __importStar(require("firebase-functions"));
const secret_manager_1 = require("@google-cloud/secret-manager");
const generative_ai_1 = require("@google/generative-ai");
const secretClient = new secret_manager_1.SecretManagerServiceClient();
let genAI = null;
let placesApiKey = null;
...
function filterByLocation(places, location) {
 ...
 */
async function batchScrapeEmails(places) {
...
async function analyzeWebsites(places) {
    if (places.length === 0)
        return [];
    ...
    const businessList = places.map(p => {
        ...
        return ({
            ...
            types: ((_a = p.types) === null || _a === void 0 ? void 0 : _a.slice(0, 3)) || []
        });
    });
 ...
 */
exports.researchBusiness = functions
    .runWith({
    timeoutSeconds: 60,
    memory: '512MB'
})
...
- Return ONLY the JSON object, no markdown or additional text`;
        ...
        if (jsonMatch) {
            ...
            if (researchData.address)
                cleanedData.address = researchData.address;
            if (researchData.phone)
                cleanedData.phone = researchData.phone;
            if (researchData.email)
                cleanedData.email = researchData.email;
            if (researchData.hours)
                cleanedData.hours = researchData.hours;
            if ((_a = researchData.services) === null || _a === void 0 ? void 0 : _a.length)
                cleanedData.services = researchData.services;
            if (researchData.rating)
                cleanedData.rating = researchData.rating;
            if (researchData.reviewCount)
                cleanedData.reviewCount = researchData.reviewCount;
            if (researchData.websiteUrl)
                cleanedData.websiteUrl = researchData.websiteUrl;
            if (researchData.googleMapsUrl)
                cleanedData.googleMapsUrl = researchData.googleMapsUrl;
            if (researchData.description)
                cleanedData.description = researchData.description;
            // Clean up social links
            if (researchData.socialLinks) {
                const socialLinks = {};
                if (researchData.socialLinks.facebook)
                    socialLinks.facebook = researchData.socialLinks.facebook;
                if (researchData.socialLinks.instagram)
                    socialLinks.instagram = researchData.socialLinks.instagram;
                if (researchData.socialLinks.twitter)
                    socialLinks.twitter = researchData.socialLinks.twitter;
                if (researchData.socialLinks.linkedin)
                    socialLinks.linkedin = researchData.socialLinks.linkedin;
                if (researchData.socialLinks.yelp)
                    socialLinks.yelp = researchData.socialLinks.yelp;
 ...
 */
exports.findBusinessesWithSearch = functions
    .runWith({
    timeoutSeconds: 60,
    memory: '512MB'
})
 ...
 */
exports.findLeadsWithMaps = functions
    .runWith({
    timeoutSeconds: 90,
    memory: '512MB'
})
    ...
    try {
        ...
        const leads = filteredPlaces.map(place => {
            ...
            const defaultStatus = place.websiteUri ? 'Outdated' : 'None';
            ...
            return {
                ...
                websiteStatus: (analysis === null || analysis === void 0 ? void 0 : analysis.websiteStatus) || defaultStatus,
                pitchAngle: (analysis === null || analysis === void 0 ? void 0 : analysis.pitchAngle) || defaultPitch,
