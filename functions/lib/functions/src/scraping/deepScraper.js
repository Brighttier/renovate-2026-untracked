...
function shouldSkipImage(src) {
    ...
    if (srcLower.includes('icon') && !srcLower.includes('fav'))
        return true;
    // Skip social media images
    if (srcLower.includes('social') && !srcLower.includes('wp-content'))
        return true;
    return false;
}
...
async function discoverInternalLinks(page, baseUrl) {
    ...
    const links = await page.evaluate((base, hostname) => {
...
// ============================================
async function extractLogoWithBase64($, page, sourceUrl) {
...
function extractTestimonials($, pages) {
    ...
    $('li').each((_, el) => {
        ...
        if (fullText.length < 100 || isFormContent(fullText))
            return;
        ...
        // Validate
        if (quote.length < 50 || quote.length > 1000)
            return;
        if (!authorName || authorName.length < 3 || authorName.length > 30)
            return;
        if (fakeNames.includes(authorName.toLowerCase()))
            return;
        if (!/^[A-Z]/.test(authorName))
            return;
        if (seenQuotes.has(quote.slice(0, 50)))
            return;
        if (isFormContent(quote))
            return;
...
// ============================================
async function extractAllColors(page, $, logoUrl, sourceUrl) {
    const colors = [];
    ...
    if (logoUrl) {
        try {
            ...
            if (response.ok) {
                ...
                if (palette.Vibrant)
                    colors.push(palette.Vibrant.hex.toUpperCase());
                if (palette.DarkVibrant)
                    colors.push(palette.DarkVibrant.hex.toUpperCase());
                if (palette.LightVibrant)
                    colors.push(palette.LightVibrant.hex.toUpperCase());
                if (palette.Muted)
                    colors.push(palette.Muted.hex.toUpperCase());
                if (palette.DarkMuted)
                    colors.push(palette.DarkMuted.hex.toUpperCase());
            }
        }
        catch (e) {
...
// ============================================
function extractImages($, sourceUrl, logoUrl) {
    const heroImages = [];
    const galleryImages = [];
    ...
    function isGoodImageUrl(url) {
        ...
        if (urlLower.includes('icon') || urlLower.includes('arrow') || urlLower.includes('star'))
            return false;
...
function extractNavigation($, sourceUrl) {
    ...
    for (const selector of navSelectors) {
        $(selector).each((_, el) => {
            ...
            if (socialPatterns.some(p => href.toLowerCase().includes(p)))
                return;
            // Normalize href
...
function extractContactInfo($) {
    ...
    if (phoneMatches === null || phoneMatches === void 0 ? void 0 : phoneMatches[0]) {
    ...
    if (emailMatches) {
        const businessEmail = emailMatches.find(e => !e.includes('example.com') && !e.includes('email.com') && !e.includes('domain.com'));
        if (businessEmail)
...
function extractBusinessName($, sourceUrl) {
    ...
    if (schemaName && schemaName.length > 2 && schemaName.length < 80)
        return schemaName;
    const title = $('title').text().trim();
    if (title) {
...
// ============================================
function extractCoreValues($, pages) {
    const values = [];
 ...
 */
async function enrichImagesWithVision(images, options = {}) {
    var _a, _b;
    const { enableOCR = true, enableColors = true, enableCaptions = true, maxImages = 15 } = options;
    ...
    try {
        ...
        const visionResults = await (0, deepVision_1.batchAnalyzeImages)(imageUrls, {
            enableOCR,
        ...
        for (let i = 0; i < images.length && i < maxImages; i++) {
            ...
            if (visionResult) {
                // Extract OCR text
                if (((_a = visionResult.textDetection) === null || _a === void 0 ? void 0 : _a.success) && visionResult.textDetection.fullText) {
                    enriched.extractedText = visionResult.textDetection.texts.map(t => t.text);
                    enriched.visionConfidence = Math.max(enriched.visionConfidence || 0, ...visionResult.textDetection.texts.map(t => t.confidence));
                    // Extract facts from OCR
                    const source = img.context === 'hero' ? 'hero' : 'signage';
                    const facts = (0, deepVision_1.extractFactsFromOCR)(visionResult.textDetection.fullText, img.url, source);
                    allFacts.push(...facts);
                }
                // Extract colors
                if (((_b = visionResult.colorExtraction) === null || _b === void 0 ? void 0 : _b.success) && visionResult.colorExtraction.colors.length > 0) {
 ...
 */
async function addSemanticCaptions(images) {
    if (images.length === 0)
        return;
    try {
 ...
 */
function buildSemanticImageMap(heroImages, galleryImages, services) {
    const map = {
        hero: [],
        services: [],
        about: [],
...
async function deepScrapeSite(url, options = {}) {
    var _a;
    const { maxPages = 8, // Reduced from 10 to 8 for modernization focus
    maxDepth = 2, timeout = 120000, 
    // Deep-Multimodal options with defaults
    enableVisionOCR = true, enableVisionColors = true, enableSemanticCaptions = true, maxImagesForVision = 15 } = options;
    ...
    try {
        ...
        const contentSparsity = fullCopy.length < 1000 ? 'sparse' :
            fullCopy.length < 3000 ? 'moderate' : 'rich';
        // ============================================
        // DEEP-MULTIMODAL: Vision API Enrichment
        // ============================================
        let enrichedHeroImages = heroImages;
        let enrichedGalleryImages = galleryImages;
        let extractedFacts = [];
        let accentColor = null;
        let visionAnalysisComplete = false;
        let semanticImageMap;
        ...
        console.log(`[DeepScraper v3.0] Extraction complete:`, {
            ...
            extractedFacts: ((_a = identity.extractedFacts) === null || _a === void 0 ? void 0 : _a.length) || 0,
...
//# sourceMappingURL=deepScraper.js.map